...
function isCreditCardNumber(ccn) {
  let x = [...String(ccn)].reverse();
  x = x.reduce((k, v, i) => {
    let digit = Number(v);
    if (i % 2) {
      digit *= 2;
    }
    let d = k + Math.floor(digit / 10);
    d += digit % 10;
    return d;
  }, 0);
  return (x * 3) % 10 === 0;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === '(' || str[i] === '[' || str[i] === '{' || str[i] === '<') {
      s.push(str[i]);
    } else if (s[s.length - 1] === map[str[i]]) {
      s.pop();
    } else return false;
  }
  return !s.length;
...
function getCommonDirectoryPath(pathes) {
  ...
  if (pathes.every((e) => e.startsWith('/'))) {
    res = `/${res}`;
  }
  return res;
}
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i <= 2; i += 1) {
    let rowEq = position[i][0] !== undefined;
    let indexR = 0;
    while (indexR <= 2 && rowEq) {
      rowEq = position[i][indexR] === position[i][0];
      indexR += 1;
    }
    if (rowEq) {
      return position[i][0];
    ...
    let columnEq = position[0][i] !== undefined;
    let index = 0;
    while (index <= 2 && columnEq) {
      columnEq = position[index][i] === position[0][i];
      index += 1;
    }
    if (columnEq) {
      return position[0][i];
